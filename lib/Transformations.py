...
from pyspark.sql.functions import col, struct, lit, array, when, isnull, filter, collect_list, expr, date_format, \
    current_timestamp
# import uuid


def get_insert_operation(column: Column, alias: str) -> Column:
...
def get_contract(df: DataFrame) -> DataFrame:
...
def get_relations(df: DataFrame) -> DataFrame:
...
def get_address(df: DataFrame) -> DataFrame:
...
def join_party_address(p_df: DataFrame, a_df: DataFrame) -> DataFrame:
    return p_df.join(a_df, "party_id", "left")\
        .groupBy("account_id").agg(collect_list(struct("partyIdentifier",
...
def join_contract_party(c_df: DataFrame, p_df: DataFrame) -> DataFrame:
    return c_df.join(p_df, "account_id", "left")


def apply_header(spark: SparkSession, df: DataFrame) -> DataFrame:
    ...
    event_df = header_df.hint("broadcast").crossJoin(df)\
        .select(struct(expr("uuid()").alias("eventIdentifier"),
                ...
                array(struct(lit("contractIdentifier").alias("keyFiled"),
